...
3. ### Create the cache
   ...
   function UserTable() {
     const cache = useTableCache<User>('users', {
      ...
      // Spread cache onto VirtualTable: {...cache}
      // This passes ref, totalCount, onRangeChange, rowHeight, and rowGap automatically
      // cache.getItem(index) — get row by index (undefined if not fetched)
   }
   ```

   :::tip
   `useTableCache` returns `rowHeight` and `rowGap` alongside `ref`, `totalCount`, and `onRangeChange`, so spreading `{...cache}` onto `<VirtualTable>` wires up all required props in one go — no need to pass `rowHeight` separately.
   :::
...
7. ### Add empty state and footer
   ...
   Add an `Empty` sub-component for when there are no results, and a `Footer` to show the current scroll position:
