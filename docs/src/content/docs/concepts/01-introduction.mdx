...
The component itself does not bundle a stylesheet — it relies on Tailwind's utility classes being available in your build pipeline. By default, Tailwind only scans your own source files for class names. Because `@requence/table` lives in `node_modules`, you need to tell Tailwind to scan it too.

Add the `@source` directive to your CSS entry point (e.g. `app.css` or `globals.css`):
...
This ensures all utility classes used by `@requence/table` are included in the generated stylesheet.
