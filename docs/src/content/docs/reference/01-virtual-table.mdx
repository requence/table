...
| `smoothScroll` | `boolean \| SmoothScrollConfig` | `true` | Enable rAF-based lerp interpolation for mouse-wheel scrolling. Pass `true` for defaults, `false` to disable, or `{ lerp?, epsilon? }` to customise. See [SmoothScrollConfig](#smoothscrollconfig). |
...
| `scrollTo` | `(px: number) => void` | Scroll to an absolute pixel offset. Cancels any in-flight smooth-scroll animation. |
| `scrollBy` | `(px: number) => void` | Adjust the scroll position by the given number of pixels. Gated by the `adjustScrollPosition` prop. |

## SmoothScrollConfig

Configuration object for the `smoothScroll` prop. All fields are optional.

```typescript
import type { SmoothScrollConfig } from '@requence/table'
```

| Property | Type | Default | Description |
|---|---|---|---|
| `lerp` | `number` | `0.08` | Lerp factor per frame — higher values feel snappier, lower values produce a smoother glide. |
| `epsilon` | `number` | `0.5` | Stop threshold in pixels — the animation stops when the remaining distance is below this value. |
