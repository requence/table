---
title: VirtualTable
description: API reference for the VirtualTable compound component — a headless, virtualized React table with CSS grid layout.
---

import { Aside } from '@astrojs/starlight/components';

`VirtualTable` is a compound component that renders a virtualized, scrollable table using CSS grid. Only the rows within (and slightly outside) the viewport are mounted, keeping DOM size constant regardless of dataset size. All layout is driven by a single CSS custom property, so every row — header, body, skeleton, empty — shares the same column grid.
...
| `children` | `ReactNode` | — | Slot components (Header, Body, SkeletonRow, Empty, Footer). |
...
| Prop | Type | Default | Description |
|---|---|---|---|
| `width` | `number \| string` | `'1fr'` | Column width. A **number** is treated as pixels. A **string** is used as a CSS grid track value (e.g. `'1fr'`, `'200px'`, `'minmax(100px, 2fr)'`). |
...
- **String** (e.g. `'1fr'`, `'2fr'`) — CSS grid fractional unit. Columns share remaining space proportionally. The default is `'1fr'`.
- For string widths, `minWidth` is used to generate a `minmax(50px, 1fr)` grid track, preventing columns from collapsing below their minimum.

#### Column Resizing

When `resizable` is `true`, a drag handle appears on the column's right edge. During the drag:

1. The column's grid track is replaced with a pixel value.
2. The pixel value is clamped between `minWidth` and `maxWidth` (and the available container width).
...
Factory functions create pre-configured slot components with default props baked in. This is useful when you want a consistent look across multiple tables without repeating the same props.
...
Each factory takes an optional `Partial` of the corresponding props type and returns a `SlotComponent` that can be used in place of the built-in compound component:
...
The table sets a CSS custom property on the outer container:

```
--vtable-grid-cols
```

This property holds the computed `grid-template-columns` value derived from all `Column` width definitions. Every row (header, body, skeleton) references this property, ensuring all rows share the exact same column grid. During a column resize, this property is updated live on the container element.

<Aside type="note">
  You generally don't need to interact with `--vtable-grid-cols` directly. It exists so that all row types — including skeleton rows that live outside the body render function — automatically inherit the same column layout.
</Aside>
