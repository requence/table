...
<Aside type="note">
  The cache state is stored in a module-level `Map` keyed by the `key` argument. This survives Suspense unmount/remount cycles. On component unmount, all entries for the key are cleaned up.
</Aside>
...
3. **Item ID is genuinely new** — The item is inserted at the correct sorted position using a binary search with the `compare` function. Pages after the insertion point are **invalidated** (deleted from cache) because their indices have shifted.
...
1. The item is removed from the `knownIds` set.
2. All cached pages are searched for the item. If found, it is spliced out.
