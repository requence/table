...
| Option | Type | Default | Description |
|---|---|---|---|
| `defaultValue` | `number \| string` | — | Default width before any resize has occurred. A **number** is pixels, a **string** is a CSS grid value (e.g. `'1fr'`, `'2fr'`). If omitted, the column uses `VirtualTable.Column`'s own default (`'1fr'`). |
